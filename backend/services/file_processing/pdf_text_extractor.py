...
def _extract_single_page_markdown(page) -> str:
    """Extract markdown from a single pdfplumber page object.
...
def extract_pdf_markdown_pagewise(file_bytes: bytes) -> List[Dict[str, Any]]:
    """Extract Markdown from PDF bytes page by page.
    
    Returns list of dicts with page_number and markdown for each page.
    Falls back to empty list if anything fails; caller will handle fallback.
    """
    if not file_bytes:
        logger.warning("extract_pdf_markdown_pagewise: empty file bytes")
        return []
    try:
        ...
        with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
            for page_num, page in enumerate(pdf.pages, start=1):
                try:
                    page_md = _extract_single_page_markdown(page)
                    pages_data.append({
                        "page_number": page_num,
                        "markdown": page_md
                    })
                    logger.debug(f"extract_pdf_markdown_pagewise: page {page_num} extracted {len(page_md)} chars")
                except Exception as e:
                    logger.warning(f"extract_pdf_markdown_pagewise: failed to extract page {page_num}: {e}")
                    # Still add the page with empty markdown
                    pages_data.append({
                        "page_number": page_num,
                        "markdown": ""
                    })
        
        total_chars = sum(len(p["markdown"]) for p in pages_data)
        logger.info(f"extract_pdf_markdown_pagewise: extracted {len(pages_data)} pages, total chars={total_chars}")
        return pages_data
    except Exception as e:
        logger.exception("extract_pdf_markdown_pagewise: pdfplumber failed: %s", e)
        return []
    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
            for page in pdf.pages:
                # Use helper function to extract single page markdown
                page_md = _extract_single_page_markdown(page)
                if page_md:
                    out_lines.append(page_md)
